...
/**
 * Issue #21 (PRD area C): confirm charges the fake ₹25 wallet before writing
...
describe('Confirm charges the wallet (e2e)', () => {
  ...
  it('charges ₹25 and leaves a payment record alongside the booked reservation', async () => {
    ...
    expect(user.walletBalance).toBe(475);
  });
  ...
  it('fails cleanly with 402 when the balance is too low: no reservation, no payment, hold released', async () => {
    ...
    // Drain the wallet below the ₹25 charge to force insufficient balance.
    await app.get(DataSource).getRepository(User).update({ id: userId }, { walletBalance: 10 });

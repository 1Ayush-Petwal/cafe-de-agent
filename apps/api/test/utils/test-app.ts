...
export async function truncateAll(app: INestApplication): Promise<void> {
  ...
  await dataSource.query(
    'TRUNCATE TABLE agent_workflows, idempotency_keys, notifications, notification_jobs, webhook_jobs, webhook_endpoints, partner_api_keys, payments, reservations, slots, tables, cafes, users RESTART IDENTITY CASCADE',
  );
}
